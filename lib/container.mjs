...
export class Container {
            ...
            if (Array.isArray(results) && results.length > 0) {
                ...
                if (existsSync(scriptPath)) {
                    ...
                    if (containerFilePath !== scriptPath) {
                        import(relativePath).then((exported) => {
                            const typeNames = Object.keys(exported);
                            const types = typeNames.map(name => exported[name]);
                            if (!types.find(t => t === constructor)) { //if class is NOT exported in the script file
                                //need to check if class on the stack extends class export from script file
                                if (!types.find(t => t === Object.getPrototypeOf(constructor))) {
                                    return;
                                }
                            }
                            const classes = getClassInfo(this);
                            this.import = { path: relativePath.toString(), classes, types };
                        });
                    }
                }
            }
    ...
    poll(callback, doneCallback) {
        const queueServiceId = setInterval(() => {
            setImmediate(async () => {
                await callback.call(this);
                if ((await doneCallback())) {
