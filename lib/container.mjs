...
export class Container {
    constructor() {
        ...
        Task.create('container', this, callStack, [
            TaskFlag.HandleErrors , TaskFlag.HighPriority , TaskFlag.OnceOff
        ]).queue(null, async function (instance, callStack) {
            instance.import = await getClassInfoForStack(instance, callStack);
        });
    }
    /**
     * @returns { String }
    ...
    */
    ...
    setReference(depInstance, type, namedInstance = null) {
        const vars = privateBag.get(this);
        vars.stack = (new Error()).stack;
        return Task.create('setReference', this, { type, depInstance, namedInstance }, [
            TaskFlag.HandleErrors, TaskFlag.MediumPriority, TaskFlag.OnceOff, TaskFlag.ValidResponse
        ]).queue(null, async function (instance, { depInstance, namedInstance }) {
            const vars = privateBag.get(instance);
            validateContainerClass(instance);
    ...
    getReference(type, namedInstance = null) {
        const vars = privateBag.get(this);
        vars.stack = (new Error()).stack;
        return Task.create('getReference', this, { type, namedInstance }, [
            TaskFlag.HandleErrors , TaskFlag.LowPriority , TaskFlag.OnceOff , TaskFlag.ValidResponse
        ]).queue(type, async function (instance, { type, namedInstance }) {
            const vars = privateBag.get(instance);
            validateContainerClass(instance);
            await validateCalledInClassContext(instance);
            if (namedInstance) {
                this.complete(vars[namedInstance]);
            } else {
    ...
    getProperty(propTemplate, type) {
        const vars = privateBag.get(this);
        vars.stack = (new Error()).stack;
        return Task.create('getProperty', this, { propTemplate, type }, [
            TaskFlag.HandleErrors , TaskFlag.LowPriority , TaskFlag.OnceOff , TaskFlag.ValidResponse
        ]).queue(type, async function (instance, { propTemplate, type }) {
            const vars = privateBag.get(instance);
            validateContainerClass(instance);
            await validateCalledInClassContext(instance);
