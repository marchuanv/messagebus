...
export class MessageBus extends Container {
    ...
    constructor(connection, channel) {
        ...
        Task.create('clientready', this, null, [
            TaskFlag.HandleErrors, TaskFlag.LowPriority, TaskFlag.RepeatUntilValidResponse
        ]).queue(null, async function (_instance) {
            const status = await _instance.getReference(MessageBusStatus.prototype);
            if (status === MessageBusStatus.Ready) {
                const client = await _instance.getReference(WebSocket.prototype, 'webSocket');
                if (client.eventNames().length === 0) {
                    client.on('close', () => {
                        _instance.setReference(MessageBusStatus.Closed, MessageBusStatus.prototype);
                        console.log(`${_instance.Id}: Websocket Client closed on port: ${destinationAddressHostPort}`);
                    });
                    client.on('error', console.error);
                    client.on('open', () => {
                        _instance.setReference(MessageBusStatus.Open, MessageBusStatus.prototype);
    ...
    */
    send(message) {
        return Task.create('send', this, message, [
            TaskFlag.HandleErrors, TaskFlag.LowPriority, TaskFlag.OnceOff, TaskFlag.RepeatUntilValidResponse
