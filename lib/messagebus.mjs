    ...
    constructor(connection, channel) {
        ...
        this.poll(async () => {
            return await this.promise(async (resolve) => {
                ...
                client.on('close', () => {
                    console.log(`${this.Id}: Websocket Client closed on port: ${destinationAddressHostPort}`);
                ...
                client.on('open', () => {
                    ...
                    console.log(`${this.Id}: Websocket Client opened on port: ${destinationAddressHostPort}`);
                    resolve(true);
                });
            });
        });
        this.promise(async () => {
            ...
            await Container.setReference(this, server, WebSocketServer.prototype, 'webSocketServerConnection');
            server.on('connection', async (con) => {
                Container.setReference(this, con, WebSocket.prototype, 'webSocketServerConnection');
                const hostPort = await connection.getPort();
                console.log(`${this.Id}: Websocket Server received a connection on port: ${hostPort}`);
            });
            server.on('close', () => {
                Container.setReference(this, MessageBusStatus.Closed, MessageBusStatus.prototype);
                console.log(`${this.Id}: Websocket Server connection was closed on port: ${hostPort}`);
            });
