...
export class MessageBus extends Container {
    ...
    constructor(connection, channel) {
        ...
        Task.create('websocketclient', this, { data: channel, priority: Priority.Medium, onceOff: false }).queue(null, async function (instance, _channel) {
            const task = this;
        ...
        Task.create('websocketserver', this, { priority: Priority.High, data: connection }).queue(null, async function (instance, _connection) {
    ...
    */
    receive() {
        return Task.create('receive', this).queue(Message.prototype, async function (instance) {
            const task = this;
    ...
    */
    send(message) {
        return Task.create('send', this).queue(null, async function (instance) {
            const status = await instance.getReference(MessageBusStatus.prototype);
            const channel = await instance.getReference(Channel.prototype);
            const isOpen = await channel.isOpen();
            if (status === MessageBusStatus.Open && isOpen) {
    ...
    */
    isStarted() {
        return Task.create('isstarted', this).queue(Boolean.prototype, async function (instance) {
            const status = await instance.getReference(MessageBusStatus.prototype);
